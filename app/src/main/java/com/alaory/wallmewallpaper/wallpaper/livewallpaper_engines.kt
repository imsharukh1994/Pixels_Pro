...
class livewallpaper_engines : WallpaperService() {


    //return my engine
    override fun onCreateEngine(): Engine {
        val type = this@livewallpaper_engines.getSharedPreferences("LiveWallpaper",0).getString("Media_Type","video");
    ...
    inner class VideoLiveWallpaperEngine : WallpaperService.Engine(){
        var exoPlayer  = ExoPlayer.Builder(this@livewallpaper_engines)
        ...
        override fun onCreate(surfaceHolder: SurfaceHolder?) {
            ...
            exoPlayer.apply {
                val prefs = this@livewallpaper_engines.getSharedPreferences("LiveWallpaper", 0);
...
//                val bottomoffset = prefs.getFloat("bottom",0f);
                ...
                //refer to https://developer.android.com/reference/androidx/media3/exoplayer/ExoPlayer#getPauseAtEndOfMediaItems()
    ...
    inner class gifengine : Engine(){
        ...
        val callbackHandler  = Handler(this@livewallpaper_engines.mainLooper);
        var scaleX = 0f;
        var scaleY = 0f;
        var largestscale = 0f;
        ...
        override fun onCreate(surfaceHolder: SurfaceHolder?) {
            ...
            when(sourceuri.scheme){
                "content" -> {
                    val  contentstream = this@livewallpaper_engines.contentResolver.openInputStream(sourceuri);
